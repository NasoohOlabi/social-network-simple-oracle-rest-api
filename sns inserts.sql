...
BEGIN
    entity_id := entity_insert(feed_owner_id
        , feed_visibility_id
        , 'feed');
    INSERT INTO feed (id) VALUES (entity_id);
    RETURN entity_id;
END;
CREATE OR REPLACE FUNCTION feed_insert
(
...
DECLARE
    chat_id NUMBER(19);
BEGIN
    INSERT INTO chat (id, kind) VALUES (chat_id, chat_kind);
    RETURN chat_id;
END;
CREATE OR REPLACE FUNCTION chat_insert
(
...
BEGIN
    INSERT INTO chatter (id, kind) VALUES (new_id, chatter_kind);
    RETURN new_id;
END;
CREATE OR REPLACE FUNCTION chatter_insert
(
...
DECLARE
    group_id NUMBER(19);
BEGIN
    group_id := entity_insert(OWNER, VISIBILITY, 'group');
...
BEGIN
    INSERT INTO notifiable (id, kind) VALUES (new_id, notifiable_kind);
    RETURN new_id;
END;
CREATE OR REPLACE FUNCTION notifiable_insert
(
...
BEGIN
    ...
    INSERT INTO entity (id, owner_id, time_created, visibility, active, kind)
    VALUES (NEW_ENTITY_ID, OWNER, SYSDATE, entity_visibility, 1, entity_kind);
...
BEGIN
    INSERT INTO post(id, feed_id, text, kind)
    VALUES (post_id, target_feed, text_value, post_kind);
    RETURN post_id;
END;
-- post isn't abstract
CREATE OR REPLACE FUNCTION post_insert
...
(
    ...
    NEW_ID NUMBER;
BEGIN
    NEW_ID := entity_insert(event_owner, event_visibility);
    INSERT INTO event (id, timing)
    VALUES (NEW_ID, event_timing);
    RETURN NEW_ID;
END;
CREATE OR REPLACE FUNCTION event_insert
(
...
BEGIN
    RETURN event_insert(
        event_timing , event_owner , visibility(event_visibility)
    );
END;
...
DECLARE
    media_id NUMBER(19);
BEGIN
    media_id := post_insert(text, target_feed, post_owner, media_post_visibility);
...
DECLARE
    share_id NUMBER(19);
BEGIN
    share_id := post_insert(text, target_feed, share_owner, visibility);
...
BEGIN
    comment_id := post_insert(text, target_feed, comment_owner, comment_visibility);
...
BEGIN
    ...
    VALUES (react_id, react_post_id, react_user_id, react_type);
    RETURN react_id;
END;
...
BEGIN
    relationship_id := notifiable_insert('user_relationship');
    INSERT INTO user_relationship (id, source_id, target_id, "TYPE", created, updated, status, notes)
    VALUES (relationship_id, sourceId, targetId, relationship_type, SYSDATE, SYSDATE, relationship_status,
            relationship_notes);
...
DECLARE
    relationship_id NUMBER(19);
BEGIN
    relationship_id := notifiable_insert('user_page_relationship_insert');
