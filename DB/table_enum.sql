...
-- visibility_user_set
CREATE OR REPLACE PACKAGE enums AS
    FUNCTION visibility
    (
        visibility_number NUMBER
    ) RETURN VARCHAR2;
    FUNCTION visibility
    (
        visibility_name VARCHAR2
    )
        RETURN NUMBER;
    FUNCTION table_kind
    (
...
END;
CREATE OR REPLACE PACKAGE BODY enums AS
    FUNCTION visibility
    (
        visibility_number NUMBER
    ) RETURN VARCHAR2
    ...
    BEGIN
        CASE visibility_number
            WHEN 1 THEN RETURN 'public' ;
            WHEN 2 THEN RETURN 'only me' ;
            WHEN 3 THEN RETURN 'friends only' ;
            WHEN 4 THEN RETURN 'friends except' ;
            WHEN 5 THEN RETURN 'only list' ;
            ELSE RETURN 'public';
            END CASE;
    END;
    ...
    BEGIN
        CASE visibility_name
            ...
            WHEN 'only me' THEN RETURN 2 ;
            WHEN 'friends only' THEN RETURN 3 ;
            WHEN 'friends except' THEN RETURN 4 ;
            WHEN 'only list' THEN RETURN 5 ;
            ELSE RETURN 1;
            END CASE;
    END;
    FUNCTION table_kind
